...
class HeroBullet:
	...
	# 杀敌机,肯定需要一个敌机作为参数传递进来
	def is_killed_enemy(self, enemy):
		"""杀死敌机"""
...
class EnemyFlight(BaseFlight):
	...
	def __init__(self, img_path, x, y, window):  # 形参
		...
		self.is_killed = False
		self.anim_index = 0  # 动画索引
		self.kill_sound = pygame.mixer.Sound("res/baozha.ogg")

	def display(self):
		if self.is_killed:  # 先判断是否被击中,然后回到初始位置(随机的位置)
			self.flight_down_anim()  # 调用击中后的动画效果

		self.window.blit(self.img, (self.x, self.y))  # 这里不能使用继承,否则会覆盖掉子类的属性初始值

	def mov_down(self):
		"""敌机移动"""
		self.y += 8  # 敌机肯定是先移动位置后进行坐标判断.
	...
	def flight_down_anim(self):
		if self.anim_index >= 21:  # 动画执行完
			self.anim_index = 0
			self.img = pygame.image.load("res/img-plane_%d.png" % random.randint(1, 7))  # 随机加载6种不同的动画效果
...
class Game:
	...
	def display(self):
		...
		for enemy_flight in enemy_list:
			...
			# 让敌机移动
			if enemy_flight.is_killed is False:
	...
	def run(self):
		...
		while True:
			...
			self.display()
			# 判断英雄飞机是否被击中
			if self.hero_flight.is_anim_down:
				self.hero_flight.is_anim_down = False
				global enemy_list
				enemy_list = []
				break
			# 键盘控制
