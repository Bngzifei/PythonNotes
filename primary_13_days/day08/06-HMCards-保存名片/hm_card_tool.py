...
def save_card():
	"""保存名片"""
	with open('cards.data','w') as f: # 文件只能写入字符串.
		f.write(str(card_list))  # 只能写入字符串,[]列表方式不行
	print('名片保存成功')


def load_card():
	"""读取名片"""
	if os.path.exists('cards.data'):  # 后缀是为了不让外行人随便打开影响程序
		with open('cards.data','r') as f:
			content = f.read()
			# print(list(content))  # 序列化,反序列化
			# print(eval(content))
			# print(type(eval(content)))
			global card_list
			card_list = eval(content)  # 使用=号就是给定义了一个新的局部变量.就是修改了变量就要global:如果不声明全局变量,就会报: Shadows name 'card_list' from outer scope
