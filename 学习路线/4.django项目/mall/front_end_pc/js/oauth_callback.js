var vm = new Vue({
    ...
    //html页面加载完就执行下面的mounted
    mounted: function(){
        // 从路径中获取qq重定向返回的code
        var code = this.get_query_string('code');
        // 注意这个路由 /oauth/qq/user/?code=
        axios.get(this.host + '/oauth/qq/user/?code=' + code, {
            ...
            .then(response => {
                if (response.data.user_id){
                    ...
                    location.href = state;  //返回它原来的地方
                } else {
                    // 用户未绑定  access_token 这个实际上是openid
                    this.access_token = response.data.access_token;
                    this.is_show_waiting = false;  //默认是隐藏,挡住了原来的该有的登录界面
                }
            })
            .catch(error => {
